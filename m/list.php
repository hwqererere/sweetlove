...
$limit = $dou->pager('article',10, $page, "list.php?cat_id=".$cat_id,' where cat_id IN (' . $cat_id .')');

/* 获取文章列表 */
$sql = "SELECT id, title, content, image, cat_id, add_time, click, description FROM " . $dou->table('article') . " WHERE cat_id IN (" . $cat_id .") ORDER BY id DESC" . $limit;

$query = $dou->query($sql);

while ($row = $dou->fetch_array($query)) {
    $url = $dou->rewrite_url('page', $row['id']);
